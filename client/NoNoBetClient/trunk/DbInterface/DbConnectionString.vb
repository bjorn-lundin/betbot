...
Public Class DbConnectionString
  ...
  Public Shared Sub DeleteAllFromRegistry()
  ...
  Public Shared Sub DeleteFromRegistryKey(connectionName As String)
    Dim regSubKey As RegistryKey = GetRegistryKeyObject()
    regSubKey.DeleteSubKeyTree(connectionName)
  End Sub


  Public Shared Function LoadFromRegistry(connectionName As String, ByRef dbConStr As DbConnectionString) As Boolean
    Dim regSubKey As RegistryKey = GetRegistryKeyObject()
    ...
    If (connectionKey.GetValue(_ServerParamName) IsNot Nothing) Then
      dbConStr.Server = Convert.ToString(connectionKey.GetValue(_ServerParamName))
    End If

    If (connectionKey.GetValue(_PortParamName) IsNot Nothing) Then
      dbConStr.Port = Convert.ToString(connectionKey.GetValue(_PortParamName))
    End If

    If (connectionKey.GetValue(_UserIdParamName) IsNot Nothing) Then
      dbConStr.UserId = Convert.ToString(connectionKey.GetValue(_UserIdParamName))
    End If

    If (connectionKey.GetValue(_PasswordParamName) IsNot Nothing) Then
      dbConStr.Password = Convert.ToString(connectionKey.GetValue(_PasswordParamName))
    End If

    If (connectionKey.GetValue(_DatabaseParamName) IsNot Nothing) Then
      dbConStr.Database = Convert.ToString(connectionKey.GetValue(_DatabaseParamName))
    End If

    If (connectionKey.GetValue(_SSLParamName) IsNot Nothing) Then
      dbConStr.SSL = Convert.ToBoolean(connectionKey.GetValue(_SSLParamName))
    End If

    If (connectionKey.GetValue(_PreloadReaderParamName) IsNot Nothing) Then
      dbConStr.PreloadReader = Convert.ToBoolean(connectionKey.GetValue(_PreloadReaderParamName))
    End If
