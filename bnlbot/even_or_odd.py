...
class EvenOrOdd(BetBot):
    """put bet on games with low odds"""

    def __init__(self, log):
        super(EvenOrOdd, self).__init__(log)
    ...
    def check_strategy(self, market_id):
        ...
        if market_id:
            ...
            if type(prices) is dict and prices['status'] == 'ACTIVE':
                ...
                self.log.info( 'odds odd : ' + str(odds_odd))
                self.log.info( 'odds even: ' + str(odds_even))
...
bot = EvenOrOdd(alog)
bot.initialize('SCORE_SUM_IS_EVEN')
