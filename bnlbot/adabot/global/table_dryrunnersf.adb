...
--------------------------------------------
  ...
  procedure Get_Values(Request : in     Request_Type;
                       Data    : in out Table_Dryrunnersf.Data_Type) is
  begin
    ...
    if Has_Value(Request, "BACKPRICE") then
      Get_Value(Request, "BACKPRICE", Data.Backprice);
    end if;
    if Has_Value(Request, "LAYPRICE") then
      Get_Value(Request, "LAYPRICE", Data.Layprice);
    end if;
    if Has_Value(Request, "RUNNERNAME") then
      Get_Value(Request, "RUNNERNAME", Data.Runnername);
    end if;
  end Get_Values;
--------------------------------------------
  ...
  procedure Set_Values(Reply  : in out Request_Type;
                       Data   : in     Table_Dryrunnersf.Data_Type) is
  begin
    Set_Value(Reply, "MARKETID", Data.Marketid);
    Set_Value(Reply, "SELECTIONID", Data.Selectionid);
    Set_Value(Reply, "INDEX", Data.Index);
    Set_Value(Reply, "BACKPRICE", Data.Backprice);
    Set_Value(Reply, "LAYPRICE", Data.Layprice);
    Set_Value(Reply, "RUNNERNAME", Data.Runnername);
  end Set_Values;
--------------------------------------------
  ...
  procedure Make_Ud4_Telegram(Request   : in out Uniface_Request.Request_Type;
    ...
    Offset      : Natural := 0;
  begin
    Construct_Ud4_Record(Request, "DRYRUNNERSF", 6, Next_Column, Offset, Operation);
    ...
    Add_Column(Request, "BACKPRICE", FLOAT_8_FORMAT, Offset);
    Add_Column(Request, "LAYPRICE", FLOAT_8_FORMAT, Offset);
    Add_Column(Request, "RUNNERNAME", STRING_FORMAT, Offset, 0);
...
--------------------------------------------
  ...
  procedure Make_Ud4_Telegram(Request   : in out Uniface_Request.Request_Type;
                              Data      : in     Table_Dryrunnersf.Data_Type;
                              Operation	: in     Operation_Type := Get_One_Record) is
  begin
...
--------------------------------------------
  ...
  function To_Xml(Data      : in Table_Dryrunnersf.Data_Type;
                  Ret_Start : in Boolean;
                  Ret_Data  : in Boolean;
                  Ret_End   : in Boolean) return String is
    Ls      : constant String := "";
    S_Start : constant String := "<DRYRUNNERSF_ROW>"  & Ls;
    S_End   : constant String := "</DRYRUNNERSF_ROW>" & Ls;
    S1 : constant String :=
    ...
    S4 : constant String :=
          "<BACKPRICE>" &  General_Routines.F8_To_String(Data.Backprice) & "</BACKPRICE>" & Ls;
    S5 : constant String :=
          "<LAYPRICE>" &  General_Routines.F8_To_String(Data.Layprice) & "</LAYPRICE>" & Ls;
    S6 : constant String :=
          "<RUNNERNAME>" & Format_String(General_Routines.Skip_Trailing_Blanks(Data.Runnername)) & "</RUNNERNAME>" & Ls;
    --------------------------------
  ...
  --------------------------------------------
  type Dryrunnersf_Reader is new Sax.Readers.Reader with record
    ...
    Dryrunnersf_Data     : Table_Dryrunnersf.Data_Type := Empty_Data;
  end record;

  overriding procedure Start_Element(Handler       : in out Dryrunnersf_Reader;
                                     Namespace_URI : Unicode.CES.Byte_Sequence := "";
                                     Local_Name    : Unicode.CES.Byte_Sequence := "";
                                     Qname         : Unicode.CES.Byte_Sequence := "";
                                     Atts          : Sax.Attributes.Attributes'Class);

  overriding procedure End_Element(Handler         : in out Dryrunnersf_Reader;
                                   Namespace_URI   : Unicode.CES.Byte_Sequence := "";
                                   Local_Name      : Unicode.CES.Byte_Sequence := "";
                                   Qname           : Unicode.CES.Byte_Sequence := "") ;

  overriding procedure Characters(Handler          : in out Dryrunnersf_Reader;
                                  Ch               : Unicode.CES.Byte_Sequence := "");

  --------------------------------------------
  procedure Start_Element(Handler       : in out Dryrunnersf_Reader;
                          Namespace_URI : Unicode.CES.Byte_Sequence := "";
                          Local_Name    : Unicode.CES.Byte_Sequence := "";
                          Qname         : Unicode.CES.Byte_Sequence := "";
  ...
  begin
    ...
    if The_Tag = Table_Dryrunnersf_Set_Name then
  ...
  --------------------------------------------
  procedure End_Element(Handler       : in out Dryrunnersf_Reader;
                        Namespace_URI : Unicode.CES.Byte_Sequence := "";
                        Local_Name    : Unicode.CES.Byte_Sequence := "";
  ...
  begin
    ...
    elsif The_Tag = Table_Dryrunnersf_Row_Name then
      if Handler.Found_Set then
        Table_Dryrunnersf.Dryrunnersf_List_Pack.Insert_At_Tail(Handler.Dryrunnersf_List, Handler.Dryrunnersf_Data);
        Handler.Dryrunnersf_Data := Empty_Data;
      end if;
    end if;
  exception
    when Ada.Strings.Length_Error => Handler.OK := False;
  ...
  --------------------------------------------
  procedure Characters(Handler          : in out Dryrunnersf_Reader;
  ...
  begin
    if Handler.Found_Set then
      if    The_Tag = MARKETID_Name then 
        Handler.Dryrunnersf_Data.Marketid := Integer_4'value(The_Value);
      elsif The_Tag = SELECTIONID_Name then 
        Handler.Dryrunnersf_Data.Selectionid := Integer_4'value(The_Value);
      elsif The_Tag = INDEX_Name then 
        Handler.Dryrunnersf_Data.Index := Integer_4'value(The_Value);
      elsif The_Tag = BACKPRICE_Name then 
        Handler.Dryrunnersf_Data.Backprice := Float_8'value(The_Value);
      elsif The_Tag = LAYPRICE_Name then 
        Handler.Dryrunnersf_Data.Layprice := Float_8'value(The_Value);
      elsif The_Tag = RUNNERNAME_Name then 
        Fix_String(The_Value, Handler.Dryrunnersf_Data.Runnername);
      end if;
    end if;
  exception
    when Ada.Strings.Length_Error => Handler.OK := False;
  ...
  procedure From_Xml(Xml_Filename : in Unbounded_String;
                     A_List       : in out Dryrunnersf_List_Pack.List_Type) is
    My_Reader   : Dryrunnersf_Reader;
    Input       : File_Input;
  begin
    My_Reader.Dryrunnersf_List := A_List;
    ...
    if not My_Reader.OK then
       Table_Dryrunnersf.Dryrunnersf_List_Pack.Remove_All(My_Reader.Dryrunnersf_List);
    end if;
    A_List := My_Reader.Dryrunnersf_List;
  end From_Xml;
