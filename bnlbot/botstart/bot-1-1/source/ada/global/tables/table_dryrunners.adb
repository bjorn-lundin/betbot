...
package body Table_Dryrunners is
  ...
  -- Primary key
  function Get(Stm : in Sql.Statement_Type) return Table_Dryrunners.Data_Type is
    Data : Table_Dryrunners.Data_Type;
  begin
    ...
    if not Sql.Is_Null(Stm, "STARTNUM") then
      Sql.Get(Stm, "STARTNUM", Data.Startnum);
    else
      Data.Startnum := (others => ' ');
    end if;
  return Data;
  end Get;
---------------------------------------------
  ...
  function Get(Marketid : Integer_4;
                       Selectionid : Integer_4) return Table_Dryrunners.Data_Type is
    Data       : Table_Dryrunners.Data_Type;
...
--------------------------------------------
  ...
  begin
    ...
    if Order then
      Sql.Prepare(Stm_Select_All_O, "select * from DRYRUNNERS order by MARKETID, SELECTIONID");
      Read_List(Stm_Select_All_O, List, Max);
    else
      Sql.Prepare(Stm_Select_All, "select * from DRYRUNNERS");
...
--------------------------------------------
  ...
  begin
    ...
    loop
      ...
      Data := Get(Stm);
      Dryrunners_List_Pack.Insert_At_Tail(List, Data);
      Count := Count +1;
    end loop;
...
--------------------------------------------
  ...
  begin
    if Start_Trans then Sql.Start_Read_Write_Transaction(Transaction); end if;
    Sql.Prepare(Stm_Select, " select * from DRYRUNNERS " & 
...
---------------------------------------------
  ...
  procedure Delete(Data : in Table_Dryrunners.Data_Type) is
  begin
    Sql.Prepare(Stm_Delete, " delete from DRYRUNNERS " & 
            "where MARKETID=:MARKETID" &
            " and SELECTIONID=:SELECTIONID" ) ;
...
--------------------------------------------
  ...
  procedure Update(Data : in out Table_Dryrunners.Data_Type; Keep_Timestamp : in Boolean := False) is
    Now     : Sattmate_Calendar.Time_Type := Sattmate_Calendar.Clock;
    Process : Process_Io.Process_Type     := Process_Io.This_Process;
  begin
    Sql.Prepare(Stm_Update, "update DRYRUNNERS set " &
            "INDEX=:INDEX," &
            "BACKPRICE=:BACKPRICE," &
            "LAYPRICE=:LAYPRICE," &
    ...
    if Data.Startnum = (Data.Startnum'Range => ' ') then
      Sql.Set_Null(Stm_Update, "STARTNUM");
    else
      Sql.Set(Stm_Update, "STARTNUM",Data.Startnum);
    end if;
...
--------------------------------------------
  ...
  procedure Insert(Data : in out Table_Dryrunners.Data_Type; Keep_Timestamp : in Boolean := False) is
    Now     : Sattmate_Calendar.Time_Type := Sattmate_Calendar.Clock;
    Process : Process_Io.Process_Type     := Process_Io.This_Process;
  begin
    ...
    Sql.Prepare(Stm_Insert, "insert into DRYRUNNERS values (" &
            ":MARKETID, " &
            ":SELECTIONID, " &
            ":INDEX, " &
            ":BACKPRICE, " &
    ...
    if Data.Startnum = (Data.Startnum'Range => ' ') then
      Sql.Set_Null(Stm_Insert, "STARTNUM");
    else
      Sql.Set(Stm_Insert, "STARTNUM",Data.Startnum);
    end if;
...
--------------------------------------------
  ...
  begin
    if (Start_Trans) then Sql.Start_Read_Write_Transaction(Transaction); end if;
    if Order then
      Sql.Prepare(Stm_Select_I1_Marketid_O, " select * from DRYRUNNERS " & 
            "where MARKETID=:MARKETID" &
      ...
      Sql.Prepare(Stm_Select_I1_Marketid, " select * from DRYRUNNERS " & 
            "where MARKETID=:MARKETID" ) ; 
  ...
  procedure Delete_I1_Marketid(Data  : in     Table_Dryrunners.Data_Type) is
  begin
      Sql.Prepare(Stm_Delete_I1_Marketid, " delete from DRYRUNNERS " & 
            "where MARKETID=:MARKETID" ) ; 
  ...
  function Is_Existing_I1(
    ...
    List       : Dryrunners_List_Pack.List_Type := Dryrunners_List_Pack.Create;
  begin
    Data.Marketid := Marketid ; 
    Read_I1_Marketid(Data, List, False, 1);
    Is_Exist := not Dryrunners_List_Pack.Is_Empty(List);
    Dryrunners_List_Pack.Release(List);
    return Is_Exist;
  end Is_Existing_I1 ;
  --------------------------------------------
  ...
  begin
    if Start_Trans then Sql.Start_Read_Write_Transaction(Transaction); end if;
    if Order then
      Sql.Prepare(Stm_Select_Runnernamestripped_O, " select * from DRYRUNNERS " & 
      ...
      Sql.Prepare(Stm_Select_Runnernamestripped, " select * from DRYRUNNERS " & 
            "where RUNNERNAMESTRIPPED=:RUNNERNAMESTRIPPED"  ) ; 
...
---------------------------------------------
  ...
  begin
    ...
    if Dryrunners_List_Pack.Is_Empty(List) then
      End_Of_Set := True;
    else
      End_Of_Set := False;
      Dryrunners_List_Pack.Remove_From_Head(List, Data);
    end if;
    Dryrunners_List_Pack.Release(List);
    end Read_One_Runnernamestripped;
---------------------------------------------

  function Count_Runnernamestripped(Data : Table_Dryrunners.Data_Type) return Integer_4 is
    use Sql;
    Count       : Integer_4 := 0;
    End_Of_Set  : Boolean := False;
    Start_Trans : constant Boolean := (Sql.Transaction_Status = Sql.None);
  ...
  begin
    if Start_Trans then Sql.Start_Read_Write_Transaction(Transaction); end if;
    Sql.Prepare(Stm_Select_Count_Runnernamestripped, "select count('a') from DRYRUNNERS where RUNNERNAMESTRIPPED = :RUNNERNAMESTRIPPED ");
...
---------------------------------------------
  procedure Delete_Runnernamestripped(Data  : in     Table_Dryrunners.Data_Type) is
  begin
      Sql.Prepare(Stm_Delete_Runnernamestripped, " delete from DRYRUNNERS " & 
            "where RUNNERNAMESTRIPPED=:RUNNERNAMESTRIPPED"  ) ; 
...
--------------------------------------------
  ...
  function To_String(Data : in Table_Dryrunners.Data_Type) return String is
  begin
    return
          ...
          " Startnum = " & General_Routines.Skip_Trailing_Blanks(Data.Startnum) &
          "";
  end To_String;
--------------------------------------------
