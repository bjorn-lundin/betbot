...
package body DOM.Core.Nodes is
   ...
   procedure Write
     (Stream                : access Ada.Streams.Root_Stream_Type'Class;
      ...
      Indentation           : Integer := 4;     --bnl
      Sort_Attributes       : Boolean := False)  --bnl 
   is
...
--bnl               Sort (N.Attributes);
               --bnl Messes up schema ?    
...
--bnl               if Collapse_Empty_Nodes and then N.Children = Null_List then
               if Collapse_Empty_Nodes and then 
                 ( N.Children = Null_List or else ( N.Children.Items (0).Node_Type = Text_Node and then Node_Value(N.Children.Items(0)) = "")) then
                  Put (Stream, Slash_Sequence & Greater_Than_Sequence, Encoding);
               else
