...
--   type Back_Price_Index_Type  is range 10 .. 100; -- will divide by 10
            ...
            case Global_Bet_Type is
               when Races.Lay =>
                 Print ("Price" & " " &
                        ...
                        "Needed_Hitrate" & " " &
                        "Needed_Hitrate_Theory" & " " &
                        "Actual_Hitrate-Needed_Hitrate" & " " &
                        "Actual_Hitrate>Needed_Hitrate" & " " &
                        "Actual_Hitrate-Needed_Hitrate_Theory" & " " &
                        "Actual_Hitrate>Needed_Hitrate_Theory" & " " &
                        "Global_Min_Saldo" & " " &
                        "Global_Saldo" & " " &
                        "Global_Max_Saldo");
--                  for Price in 2 .. 24 loop
                ...
                loop
                        ...
                        Global_Min_Price := Global_Min_Price + 5.0;
                        Global_Max_Price := Races.Max_Price_Type(Global_Min_Price + 5.0) ;
                        exit when Global_Min_Price > 50.0;
...
--                        end if; -- min_price < Max_Price
                        ...
                        if Global_Num_Bets > 0 then
                          ...
                          Needed_Hitrate_Theory := 100.0 * Hitrates.Needed_Laybet_Hitrate(Global_Avg_Price,Hitrates.No_Commission);
                        else
                          Global_Avg_Price := 0.0;
                          Actual_Hitrate := 0.0 ;
                          Needed_Hitrate := 0.0;
                          Needed_Hitrate_Theory  := 0.0;
                        end if;

                        Print (Integer (Global_Min_Price)'Img & " " &
                             Integer (Global_Max_Price)'Img & " " &
                             ...
                             Actual_Hitrate'Img & " " &
                             Needed_Hitrate'Img & " " &
                             Needed_Hitrate_Theory'Img & " " &
                             integer'image(integer(Actual_Hitrate - Needed_Hitrate)) & " " &
                             Boolean'Image (Actual_Hitrate > Needed_Hitrate) & " " &
                             integer'image(integer(Actual_Hitrate - Needed_Hitrate_Theory)) & " " &
                             Boolean'Image (Actual_Hitrate > Needed_Hitrate_Theory) & " " &
...
--                  for Price in 2 .. 24 loop
                ...
                loop
                        ...
                        Global_Min_Price := Global_Min_Price + 5.0;
                        Global_Max_Price := Races.Max_Price_Type(Global_Min_Price + 5.0) ;
                        exit when Global_Min_Price > 25.0;
 --                    for Min_Price in Min_Price_Index_Type'Range loop
                        Global_Num_Races    := 0;
                        Global_Num_Bets     := 0;
                        Global_Num_Bets_Won := 0;
...
--                     for Delta_Price in Delta_Price_Index_Type'Range loop
                  ...
                  for Price in 1 .. 10 loop
                        ...
                        loop
                           ...
                           Race.Make_Back_Bet
                             (Animal            => Global_Animal,
                              ...
                              Size              => Global_Size,
                              Back_Price        => Races.Back_Price_Type(Price),
                              Delta_Price       => 0.25);
                        ...
                        if Global_Num_Bets > 0 then
                          ...
                          Needed_Hitrate_Theory := 100.0 * Hitrates.Needed_Backbet_Hitrate(Global_Avg_Price,Hitrates.No_Commission);
                        else
                          Global_Avg_Price := 0.0;
                          Actual_Hitrate := 0.0 ;
                          Needed_Hitrate := 0.0;
