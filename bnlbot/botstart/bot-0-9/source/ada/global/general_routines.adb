...
package body General_Routines is
  ...
  ---------------------------------------------------------------------------
   ...
   --The lexical representation for duration is the [ISO 8601] extended format PnYn MnDTnHnMnS, 
  ...
  --Here, it is all less than a day, and no milliseconds. 

--  function XML_Duration(Dur : String) return Sattmate_Calendar.Time_Type is
--    Duration_T_Value  : Schema.Date_Time.Duration_T;
--    Calendar_Time     : Ada.Calendar.Time;
--    Year,Month,Day    : Natural := Natural'first;
--  begin
--    Duration_T_Value := Schema.Date_Time.Value(Dur);
--    Year := Schema.Date_Time.Year(Duration_T_Value);
--    Month := Schema.Date_Time.Month(Duration_T_Value);
--    Day := Schema.Date_Time.Day(Duration_T_Value);
--
--    if Year < Ada.Calendar.Year_Number'First then
--      Year := Ada.Calendar.Year_Number'First;
--    end if;
--    if Year > Ada.Calendar.Year_Number'Last then
--      Year := Ada.Calendar.Year_Number'Last;
--    end if;
--    if Month < Ada.Calendar.Month_Number'First then
--      Month := Ada.Calendar.Month_Number'First;
--    end if;
--    if Month > Ada.Calendar.Month_Number'Last then
--      Month := Ada.Calendar.Month_Number'Last;
--    end if;
--    if Day < Ada.Calendar.Day_Number'First then
--      Day := Ada.Calendar.Day_Number'First;
--    end if;
--    if Day > Ada.Calendar.Day_Number'Last then
--      Day := Ada.Calendar.Day_Number'Last;
--    end if;
--
--    Calendar_Time := Ada.Calendar.Time_Of
--       (Year    => Year,
--        Month   => Month,
--        Day     => Day,
--        Seconds => Schema.Date_Time.Seconds(Duration_T_Value));
--
--    return Sattmate_Calendar.To_Time(Calendar_Time);
--  exception
--    when Ada.Calendar.Time_Error => raise Invalid_Date_Or_Time_Part;
--  end XML_Duration;
--  ---------------------------------------------------------

  function XML_Duration(Dur : Sattmate_Calendar.Time_Type) return String is
      use Sattmate_Calendar;
  --    use type Sattmate_Types.Integer_2;
      Local_String : String := "PT  H  M  S";
      H_Is_Blank,
      M_Is_Blank : Boolean := False;
  begin
    declare
      String_Hour : String := General_Routines.Trim(Hour_Type'Image(Dur.Hour));
    begin
      if Dur.Hour > 0 then
        case String_Hour'Length is 
          when 1 => Local_String(4)    := String_Hour(1); 
          when 2 => Local_String(3..4) := String_Hour(1..2); 
          when others => raise Constraint_Error; 
        end case;
      else
        Local_String(5) := ' '; -- blank out the 'H' 
        H_Is_Blank := True;
      end if;
    end;

    declare
      String_Minute : String := General_Routines.Trim(Minute_Type'Image(Dur.Minute));
    begin
      if Dur.Minute > 0 then
        case String_Minute'Length is 
          when 1 => Local_String(7)    := String_Minute(1); 
          when 2 => Local_String(6..7) := String_Minute(1..2); 
          when others => raise Constraint_Error; 
        end case;
      else
        Local_String(8) := ' '; -- blank out the 'M' 
        M_Is_Blank := True;
      end if;
    end;

    declare
      String_Second : String := General_Routines.Trim(Second_Type'Image(Dur.Second));
    begin
      if Dur.Second > 0 then
        case String_Second'Length is 
          when 1 => Local_String(10)    := String_Second(1); 
          when 2 => Local_String(9..10) := String_Second(1..2); 
          when others => raise Constraint_Error; 
        end case;
      else
        -- we can only blank out the 'S' if H or M is still there
        if not H_Is_Blank or else
           not M_Is_Blank then
          Local_String(11) := ' '; -- blank out the 'S' 
        else
          Local_String(10..11) := "0S"; -- say 0 seconds, so it is not empty 
        end if; 
      end if;
    end;
    return General_Routines.Skip_All_Blanks(Local_String);
  end XML_Duration; 

  ---------------------------------------------------
  
  function XML_Time(Time : String) return Sattmate_Calendar.Clock_Type is
    Local_Clock : Sattmate_Calendar.Clock_Type;
  begin -- the incoming 'Time' param assumes is validated
    Local_Clock.Hour := Sattmate_Calendar.Hour_Type'Value(Time(1..2));
    Local_Clock.Minute := Sattmate_Calendar.Minute_Type'Value(Time(4..5));
    Local_Clock.Second := Sattmate_Calendar.Second_Type'Value(Time(7..8));
    return Local_Clock;
  end XML_Time;
  
  function XML_Time(Time : Sattmate_Calendar.Clock_Type) return String is
  ...
  begin
    ...
    return XML_Duration(Local_Time);
  end XML_Time;  
