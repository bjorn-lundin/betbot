select
  ...
  min(b.betplaced)::date as mindate,
  max(b.betplaced)::date as maxdate,
  max(b.betplaced)::date - min(b.betplaced)::date  + 1 as days, 
  round(count('a')/(max(b.betplaced)::date - min(b.betplaced)::date  + 1)::numeric,2) as betsperday, 
  round((sum(profit)/(max(b.betplaced)::date - min(b.betplaced)::date  + 1))::numeric, 2) as profitperday,
  e.countrycode,
  b.betname --,
...
where
  b.betplaced::date > (select current_date - interval '42 days')
