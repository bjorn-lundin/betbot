...
procedure Bet_During_Race_2 is

  package Ev renames Ada.Environment_Variables;
  use type Rpc.Result_Type;
  use type Ada.Containers.Count_Type;

  Me : constant String := "Bet_During_Race_2.";
  ...
  begin
    ...
    if History_Exists(Winner) then
      ...
      Tic_Loop  : for Bet_Tic in Betting_Limit_Type'Range loop
        Lay_Tick := Tics.Get_Tic_Index(Price_Data.Layprice);
        Bet_Price := Tics.Get_Tic_Price(Bet_Tic + Lay_Tick);
        ...
        begin
          ...
          Race_Win : for Race_Data of Price_During_Race_List(Winner) loop
            if Price_Is_Ok(Race_Data) then
              ...
              case Bet_State is
                when Started => --start with laying with PRICE_DATA
...
begin
  ...
  Log(Sa_Min_Layprice.all & "' '" & Sa_Max_Layprice.all & "'");
