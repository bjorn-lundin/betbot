...
procedure Lay_And_Back_After_Progress is
  ...
  begin
    ...
    return "WTF-LAYBACK_AFTER_" & F8_Image(Start_Price) & "_" & F8_Image(Bet_Price);
  end Name;
  ...
  procedure Run(Price_Data : in Prices.Price_Type;
    ...
    Laybet                 : array (Bot_Types.Bet_Market_Type ) of Bets.Bet_Type;
  ...
  begin
    ...
    if History_Exists(Winner) then
      ...
      for Delta_Tics in Tics.Tics_Type'Range loop
        ...
        -- make a laybet
        Bet_Name.Set(Name(Start_Price => Price_Data.Backprice, Bet_Price => B_Price));
        Move("WIN_" & Bet_Name.Fix_String,Bn);
        Sim.Place_Bet(Bet_Name         => Bn,
                      ...
                      Bet_Placed       => Market(Winner).Startts,
                      Bet              => Laybet(Winner) ) ;
        Move("M",Laybet(Winner).Status);
        Laybet(Winner).Check_Outcome(Runner(Winner));
        Laybet(Winner).Insert;

        if History_Exists(Place) then
          declare
            Pd_Lay : Prices.Price_Type;
            Eos    : Boolean := False;
          begin
            ...
            if Race_Data.Backprice > Fixed_Type(1.0)   and then -- must be valid
              ...
              if Race_Data.Pricets  >= Price_Data.Pricets and then
                ...
                if Race_Data.Backprice >= B_Price then     -- must be same or better for match
                  Move("WIN_" & Bet_Name.Fix_String,Bn);
                  Backsize := Laysize * Bet_Size_Type(Laybet(Winner).Price/B_Price);
          ...
          Race_Plc : for Race_Data of Price_During_Race_List(Place) loop
            ...
            if Race_Data.Backprice > Fixed_Type(1.0)   and then -- must be valid
              ...
              if Race_Data.Pricets >= Match_Time + (0,0,0,1,0) then
                ...
                Move("PLC_" & Bet_Name.Fix_String,Bn);
                Backsize := Laysize * Bet_Size_Type(Laybet(Place).Price/B_Price);
                Sim.Place_Bet(Bet_Name         => Bn,
                              Market_Id        => Market(Place).Marketid,
                              Side             => Back,
                              Runner_Name      => Runner(Place).Runnernamestripped,
  ...
  Global_Size      : constant Bet_Size_Type := 100.0;
  ...
  begin
    ...
    begin
      for Price of Price_List loop -- all runners in race
        ...
        T.Start;
        Run(Price_Data => Price, Laysize => Global_Size);
        T.Commit;
      end loop;
    end;
  end;
...
exception
  ...
  when E: others =>
    ...
    Logging.Close;
end Lay_And_Back_After_Progress;
