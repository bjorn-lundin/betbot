...
procedure Bot_Web_Server is
  ...
  begin            
   ...
   exception      
     when Process_Io.System_Error => null;      
       Semaphore.Release;
   end Wait_Terminate;
...
begin
 ...
 -- Logging.Log(Cfg.To_String);
  Ini.Load(Ev.Value("BOT_HOME") & "/" & "login.ini");


  Logging.Log("Main", "AWS " & AWS.Version & " starting at port 9080" );
--  AWS.Config.Set.Session_Cleanup_Interval(Value => Bot_Ws_Config.Get_Session_Cleanup_Interval);
--  AWS.Config.Set.Session_Lifetime        (Value => Bot_Ws_Config.Get_Session_Lifetime);
