...
--------------------------------------------------------------------------------
  ...
  begin
    case System_Number_Type is
      when Betid       => Select_Table_Statements(System_Number_Type).Prepare(
                            "select BETID from ABETS where BETID = :NUM");
    end case;
...
--------------------------------------------------------------------------------
  ...
  begin
    ...
    case Sql.Database is
      when Sql.PostgreSQL =>
        case System_Number_Type is
          when Betid =>  Sql.Prepare(Get_Number(System_Number_Type), "select nextval('BET_ID_SERIAL')");
        end case;
    end case;
