...
-----------------------------------------------------------------------
--CHG-21064 2011-01-25 BNL When converting &amp; invalid_encoding was raised on aix,
--                         due to wrong BOM interpretation
-----------------------------------------------------------------------
...
package body Unicode.CES.Utf32 is
   ...
   begin
      ...
      case BOM is
         ...
         when UCS4_LE  => O := Low_Byte_First;  --bnl CHG-21064
         when Ucs4_BE  => O := High_Byte_First; --bnl CHG-21064     
         when Unknown  => null;
         when others   => raise Invalid_Encoding with "BOM = " & Bom_Type'Image(BOM); --bnl CHG-21064
      end case;
