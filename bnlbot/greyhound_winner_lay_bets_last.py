...
class GreyHoundWinnerLayBetLastBot(BetBot):
    """put bet on games with low odds"""

    def __init__(self, log):
        super(GreyHoundWinnerLayBetLastBot, self).__init__(log)
############################# end __init__
...
#                lay_price = None
                ...
                for list_item in sorted_list :
                    ...
                    self.log.info( 'SORTED back/lay/selection/idx ' + \
                            str(list_item)                      )
                    if (number_of_runners >= 6 and  # GBR num of hounds
                        i == turns
                        ) :
...
bot = GreyHoundWinnerLayBetLastBot(alog)
bot.initialize('HOUNDS_WINNER_LAY_BET_LAST')
